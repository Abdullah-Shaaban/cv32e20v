...
extern int8_t image[] __attribute__((aligned(4)));    // [ R*C ]
extern int8_t filter[] __attribute__((aligned(4)));   // [ F*F ]
extern int8_t output[] __attribute__((aligned(4)));   // [ (R-F+1)*(C-F+1) ]
extern int8_t golden_o[] __attribute__((aligned(4))); // [ (R-F+1)*(C-F+1) ]
...
int main(){
  ...
  conv2d_3x3(output, image, filter, R-F+1, C-F+1);
  ...
  // Print output matrix
  printf("\nOutput:\n");
  print_matrix(output, R-F+1, C-F+1);

  // Verify correctness
  printf("Verifying result...\n");
  int error = 0;
  for (int indx = 0; indx < (R-F+1) * (C-F+1); indx++) {
